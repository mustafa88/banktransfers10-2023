...
@section('page-content')
    ...
    <div class="card">
        ...
        <fieldset>
            <form method="post" action="{{route('table.connect_projects_income.edit',$project['id'])}}">
                @csrf
            <div class="col-12">
        @foreach($income as $item)
            <label class="c-checkbox">
                <input type="checkbox" name="selctincome[]" value="{{$item['id']}}"
                       @if (in_array($item['id'], array_column($project['income'], 'id')) ) checked @endif            >
